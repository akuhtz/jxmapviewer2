...
/**
 * Uses Google Maps API - it has several glitches, so don't use it unless you
 * know what you're doing. Most importantly, gmaps cuts of small text leftovers
 * that reach into the visible tile. As a result you cannot really tile the
 * rendered tiles.
 */
public class GoogleMapsTileFactoryInfo extends TileFactoryInfo {
	...
	/**
	 * Currently only 256x256 works - see
	 * https://github.com/msteiger/jxmapviewer2/issues/62
	 */
	...
	public GoogleMapsTileFactoryInfo(String name, String baseURL, String key) {
		super(name, 1, max - 2, max, TILE_SIZE, true, true, baseURL, "x", "y", "z");
		this.key = key;
	}
	...
	public String getTileUrl(int x, int y, int zoom) {
		...
		String url = baseURL + "?center=" + lat_deg + "," + lon_deg + "&zoom=" + zoom + "&key=" + key + "&maptype=roadmap" + "&size=" + TILE_SIZE + "x" + TILE_SIZE;
